...
def normalize_path_input(path_input: str) -> str:
    """
    Normalize path input to handle Windows path formats and quotes.

    Handles:
    - Removes surrounding quotes (single or double)
    - Normalizes path separators (backslash to forward slash)
    - Handles escaped backslashes (double backslash)
    - Preserves special variables like {KVERUS_PATH}
    ...
    # Handle special variables，just normalize basic separators
    if "{" in path and "}" in path:
        path = path.replace("\\\\", "/").replace("\\", "/")
        return path

    # For regular paths, do full normalization
    try:
        # Handle escaped backslashes and normalize separators
        path = path.replace("\\\\", "\\")  # Convert double backslashes to single

        # Convert backslashes to forward slashes for consistency
        # Forward slashes work on both Windows and Unix systems
        path = path.replace("\\", "/")

        # Remove any duplicate slashes (except for UNC paths like //server/share)
        ...
        if path and not is_reasonable_path(path):
            logger.warning(
                f"Path '{path}' may not be valid. Please verify it's correct."
            )

        return path

    except Exception as e:
        logger.warning(f"Failed to normalize path '{path_input}': {e}. Using as-is.")
        return path_input


def is_reasonable_path(path: str) -> bool:
    """
    Basic validation to check if a path looks reasonable.

    Args:
        path: Path to validate

    Returns:
        True if path looks reasonable, False otherwise
    """
    # Allow empty paths
    if not path:
        return True
    ...
    # Allow: absolute paths, relative paths, UNC paths, template variables
    reasonable_patterns = [
        path.startswith("/"),  # Unix absolute
        path.startswith("\\"),  # Windows UNC
        len(path) >= 2 and path[1] == ":",  # Windows drive letter
        path.startswith("./") or path.startswith("../"),  # Relative paths
        "{" in path and "}" in path,  # Template variables
        not path.startswith("/")
        and not path.startswith("\\")
        and ":" not in path,  # Relative without ./
    ]

    return any(reasonable_patterns)


@cache
def expand_config_path(path: str) -> str:
    """
    Expand configuration placeholders in `path`.

    Assumes `path` is already normalized.

    Supported placeholders:
      - {KVERUS_PATH}: replaces with `global_vars.kverus_path` or repo root
      - {VERUS_DIR}: replaces with `global_vars.config['paths']['verus_dir']`
      - {VERUS_BIN}: expands to the resolved verus binary path
    ...
    # KVERUS_PATH
    if "{KVERUS_PATH}" in result:
        kverus_path = getattr(global_vars, "kverus_path", None)
        if kverus_path:
            result = result.replace("{KVERUS_PATH}", str(kverus_path))
        else:
            repo_root = Path(__file__).resolve().parent.parent
            result = result.replace("{KVERUS_PATH}", str(repo_root))

    if "{VERUS_DIR}" in result or "{VERUS_BIN}" in result:
        try:
            verus_dir_cfg = global_vars.config.get("paths", {}).get("verus_dir", "")
        except Exception:
            verus_dir_cfg = ""

        if verus_dir_cfg:
            if "{KVERUS_PATH}" in verus_dir_cfg:
                verus_dir_cfg = expand_config_path(verus_dir_cfg)
        else:
            logger.error(
                "{VERUS_DIR} is not configured in config.toml. Please set 'paths.verus_dir'."
            )
            sys.exit(1)

    # VERUS_DIR
    if "{VERUS_DIR}" in result:
        result = result.replace("{VERUS_DIR}", verus_dir_cfg)

    # VERUS_BIN
    if "{VERUS_BIN}" in result:
        # Resolve the verus binary from the verus_dir
        resolved = _find_verus_binary_internal(verus_dir_cfg)
        result = result.replace("{VERUS_BIN}", str(resolved))
    return result


@cache
def find_verus_binary(verus_dir_path: str) -> Path:
    """
    Resolve the actual verus project directory and find the binary.
    The placeholders in `verus_dir_path` will be expanded and the verus binary is searched in:
    1. verus_dir_path/verus.exe or verus_dir_path/verus
    2. verus_dir_path/source/target-verus/release/verus.exe or verus
    The program will return an empty Path if not found.

    :param verus_dir_path: The path to the verus project directory
    :return: The path to the verus binary
    """
    expanded_path = expand_config_path(verus_dir_path)
    return _find_verus_binary_internal(expanded_path)


@cache
def _find_verus_binary_internal(verus_dir_path: str) -> Path:

    verus_dir = Path(verus_dir_path)

    if not verus_dir.exists():
        logger.error(f"Verus directory does not exist: {verus_dir}")
        return Path("")

    if not verus_dir.is_dir():
        logger.error(f"Verus path is not a directory: {verus_dir}")
        return Path("")

    # Try direct directory first
    for name in ["verus.exe", "verus"]:
        candidate = verus_dir / name
        if candidate.exists() and candidate.is_file():
            logger.info(f"Found verus binary: {candidate}")
            return candidate

    # Try source/target-verus/release directory
    verus_target_dir = verus_dir / "source" / "target-verus" / "release"
    for name in ["verus.exe", "verus"]:
        candidate = verus_target_dir / name
        if candidate.exists() and candidate.is_file():
            logger.info(f"Found verus binary: {candidate}")
            return candidate

    logger.error(f"Could not find verus binary in {verus_dir}. ")
    return Path("")
...
def setup_default_config(fvt_name: str | None):
    """
    ...
    # get library name
    if fvt_name is None:
        if len(global_vars.fvts.keys()) == 1:
            fvt_name = list(global_vars.fvts.keys())[0]
        else:
            logger.critical("Please specify the library name with the -F/--fvt option")
            sys.exit(1)

    # check if the library is in the libraries.toml
    if fvt_name not in global_vars.fvts.keys():
        logger.critical(f"FVT {fvt_name} not found in the fvts.toml.")
        sys.exit(1)

    # load the library configuration template as default value
    global_vars.fvts[fvt_name] = deep_merge(
        global_vars.fvts_template["a_verus_fvt"],
        global_vars.fvts[fvt_name],
    )

    # load the KVerus configuration template as default value
    ...
    global_vars.fvt_name = fvt_name
    global_vars.fvt_config = global_vars.fvts[fvt_name]

    # Expand placeholders in fvt config paths
    for key in ["crate_path", "output_path", "database_path"]:
        if key in global_vars.fvt_config and isinstance(
            global_vars.fvt_config[key], str
        ):
            global_vars.fvt_config[key] = expand_config_path(
                global_vars.fvt_config[key]
            )
        elif key in global_vars.fvt_config and isinstance(
            global_vars.fvt_config[key], list
        ):
            global_vars.fvt_config[key] = [
                expand_config_path(p) for p in global_vars.fvt_config[key]
            ]

    # resolve some path for latter process
    # use `strict=False` here to tolerate output_path not exist
    _path = Path(global_vars.fvt_config.get("output_path")).resolve(strict=False)
    global_vars.fvt_config["output_path"] = str(_path)

    # set library language
    if global_vars.fvt_config["language"] in ["rust", "Rust", "RUST"]:
        global_vars.fvt_language = global_vars.SupportedLanguages.RUST
    else:
        logger.critical(
            f"Language {global_vars.fvt_config['language']} is not supported."
        )
        sys.exit(1)

    # set FVT tool
    if global_vars.fvt_config["fvt_tool"] in ["verus", "Verus", "VERUS"]:
        global_vars.fvt_tool = global_vars.SupportedTools.VERUS
    else:
        logger.critical(
            f"Verify tool {global_vars.fvt_config['fvt_tool']} is not supported."
        )
        sys.exit(1)


def get_llm_config(llm_name: Optional[str]) -> tuple[dict, str, str]:
    """
    Get the LLM configuration by name

    :param llm_name: The name of the LLM configuration
    :return: selected_llm, llm_type, llm_name
    """
...
def setup_llm(llm_name: str) -> LLM.LLMClient:
    ...
    selected_llm, llm_type, llm_name = get_llm_config(llm_name)
...
class FilePos:
    ...
    def __init__(self, file: Path, line: int, col: int, reload_file: bool = True):
        """
    ...
    @staticmethod
    def from_location_line(location_line: str, reload_file: bool = True) -> "FilePos":
        """
