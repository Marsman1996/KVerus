...
def dict_to_tomlkit(data, doc, doc_section=None):
...
def save_config_file(config_path: Path, config_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        dict_to_tomlkit(config_data, doc)
...
def dump_config_str(config_data: Dict[str, Any]) -> str:
    """Dump configuration data to a TOML string without comments."""
    try:
        # Create clean TOML document
        doc = tomlkit.document()
        ...
        logger.error(f"Failed to dump configuration data to string: {e}")
        return ""
