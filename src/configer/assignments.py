...
from src.utils import normalize_path_input
...
def set_module_llm(config_path: Path, module: str, llm_name: str) -> None:
    ...
    if not config_path.exists():
        ...
        logger.info("Run 'kverus config init' to create a configuration file")
        sys.exit(1)
...
def set_path(config_path: Path, path_name: str, path_value: str) -> None:
    """Helper function to set path value in paths section."""
    # Normalize the path input
    normalized_path = normalize_path_input(path_value)
    ...
    # Get old value and set new value
    old_value = config_data["paths"].get(path_name, "Not set")
    config_data["paths"][path_name] = normalized_path

    # Save configuration
    if save_config_file(config_path, config_data):
        logger.success(
            f"Set {path_name} path to '{normalized_path}' (was: {old_value})"
        )
    else:
        sys.exit(1)
