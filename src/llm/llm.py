    ...
    def _print_log(self, content: str):
        ...
        with self.lock:
            if not hasattr(self, "log_file"):
                self.log_file = open(
                    global_vars.kverus_path / "logs" / "llm.log",
                    "a",
                    encoding="utf-8",
                )
            self.log_file.write(
        ...
        try:
            ...
            if self.is_stream:
                ...
                for chunk in stream:
                    ...
                    full_response += delta
                    # try to capture final usage if provided
                    if getattr(chunk, "usage", None) is not None:
                        usage = chunk.usage
                response = full_response
                if usage is None:
                    # fall back to manual token counting if no usage
                    prompt_tokens = 0
                    for m in messages:
    ...
    def query_with_messages(
        ...
        try:
            ...
            if self.is_stream:
                ...
                full_response = ""
                full_reasoning = ""
                # for reasoning models, reasoning content might be mixed in <think> tags
                for chunk in stream:
                    if not chunk.choices:
                        continue
            ...
            elif completion is not None and hasattr(
        ...
        if not return_tokens:
            ...
            try:
                if completion is not None:
                    ...
                    # streaming path without precise usage; approximate with word count
                    prompt_tokens = 0
                    for m in messages:
                        if isinstance(m, dict) and "content" in m:
                            content = m["content"]
                            if isinstance(content, str):
                                prompt_tokens += len(content.split())
                    completion_tokens = len((reasoning + response).split())
            except Exception:
                prompt_tokens = 0
                completion_tokens = 0
...
class LLMChat:
    ...
    def token(self) -> tuple[int, int]:
        """
        :return: Currently token consumption (prompt tokens, completion tokens)
        """
        return (
            self.client.query_logger.stats.total_tokens[0],
            self.client.query_logger.stats.total_tokens[1],
        )
