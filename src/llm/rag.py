...
class RAGRetriever(ABC):
    ...
    def add_list_texts(self, texts: list[str], sources: list[str], document: Path):
        """
        Add a list of texts to the retriever

        :param texts: List of texts
        """
        if self.is_in_database(document):
            logger.warning(f"Texts {texts} already exist in the retriever")
            return

        from src.utils import ProgressTitle

        with ProgressTitle(f"Loading texts from {document}..."):
            docs = [
                Document(page_content=t, metadata={"source": s})
