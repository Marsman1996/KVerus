...
class Knowledge:
    ...
    def from_config(cls, config: dict, library_config: dict):
        ...
        :param config: The KVerus configuration.
...
class ModelKnowledge(Knowledge):
    """
    Class to store model knowledge, i.e. documents and their retriever.
    """

    def __init__(self, info_path: Path, rag: RAGRetriever):
        """
        Initialize the knowledge with the document paths and the RAG retriever.

        :param info_path: The path to the model info.
        :param rag: The initialized RAG retriever.
        """
        if not info_path.exists():
            logger.critical(
                f"Failed to load library preprocessing results, please run preprocessor first: {info_path}"
            )
            traceback.print_exc()
            sys.exit(1)
        func_infos = json.loads(info_path.read_text())["function_infos"]
        texts = []
        sources = []
        for func_loc, func_info in func_infos.items():
            if func_info["isPub"] == True:
                texts.append(func_info["signature"])
                sources.append(func_loc)
        self.rag = rag
        if not self.rag.is_in_database(info_path):
            self.rag.add_list_texts(texts, sources, info_path)

    @classmethod
    def from_config(cls, config: dict, fvt_config: dict):
        """
        Initialize the knowledge from the configuration.

        :param config: The KVerus configuration.
        :param fvt_config: The FVT configuration.
        """
        # load configs and setup RAG
        preprocess_path = Path(fvt_config["output_path"]) / "preprocessor"
        info_path = preprocess_path / "info.json"

        database_path = Path(fvt_config["output_path"]) / "comprehender" / "lemmas"
        rag = setup_rag(config["comprehender"]["embedding_llm"], database_path)
        return cls(info_path, rag)

    @classmethod
    def load(cls, config: dict, database_path: Path):
        """
        Load the knowledge from the database.

        :param config: The KVerus configuration.
        :param database_path: The path to the database.
        """
        rag = setup_rag(config["comprehender"]["embedding_llm"], database_path)
        # FIXME: could there be multiple documents?
        info_path = Path(json.loads((database_path / "documents.json").read_text())[0])
        return cls(info_path, rag)
