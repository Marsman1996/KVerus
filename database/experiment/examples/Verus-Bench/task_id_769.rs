            ...
            0 <= i < j < result.len() ==> #[trigger] result[i] != #[trigger] result[j],
{
    ...
    while index < arr1.len()
        invariant
            ...
            forall|x: i32| result@.contains(x) ==> (!arr2@.contains(x) && (exists|k: int| 0 <= k < index && arr1[k] == x)),
    ...
    while index < arr2.len()
        invariant
            ...
            forall|x: i32| result@.contains(x) ==> (!arr2@.contains(x) && (exists|k: int| 0 <= k < arr1.len() && arr1[k] == x)) || (!arr1@.contains(x) && (exists|k: int| 0 <= k < index && arr2[k] == x)),
